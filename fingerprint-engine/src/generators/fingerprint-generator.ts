...
export class FingerprintGenerator {
  ...
  private static getPlatform(uaResult: UAParser.IResult): string {
    const os = uaResult.os.name;
    if (os?.includes('Windows')) return 'Win32';
    if (os?.includes('Mac')) return 'MacIntel';
    if (os?.includes('Linux')) return 'Linux x86_64';
    return 'Win32';
  }

  private static getPixelRatio(uaResult: UAParser.IResult): number {
    if (uaResult.device.type === 'mobile') return 2;
    if (uaResult.os.name?.includes('Mac')) return 2;
    return 1;
  }

  private static getAvailableResolution(resolution: string): string {
    const [width, height] = resolution.split('x').map(Number);
    const taskbarHeight = this.getRandomInt(30, 50);
    return `${width}x${height - taskbarHeight}`;
  }

  private static getWebGLRenderer(vendor: string): string {
    const renderers = this.webglRenderers[vendor] || ['WebKit WebGL'];
    return this.getRandomElement(renderers);
  }

  private static generateCanvasFingerprint(): CanvasFingerprint {
    const dataURL = `data:image/png;base64,${this.generateRandomBase64(100)}`;
    const noise = Math.random() * 0.0001;
    const hash = CryptoJS.SHA256(dataURL + noise).toString();
  ...
  private static generateAudioFingerprint(): AudioFingerprint {
    return {
  ...
  private static generatePlugins(uaResult: UAParser.IResult): PluginData[] {
  ...
  private static generateWebRTCFingerprint(): WebRTCFingerprint {
    ...
    return {
      mode,
      publicIP: mode === 'real' ? undefined : faker.internet.ip(),
      localIPs: [
  ...
  private static generateBatteryFingerprint(): BatteryFingerprint {
  ...
  private static generateConnectionFingerprint(): ConnectionFingerprint {
    return {
  ...
  static calculateSimilarity(fp1: BrowserFingerprint, fp2: BrowserFingerprint): number {
    ...
    if (fp1.canvas.hash === fp2.canvas.hash) score += weights.canvas;
